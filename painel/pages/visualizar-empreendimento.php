<?php 
 if(isset($_GET['id'])){
    ...
    Painel::alert('erro','Faltou o parametro ID');
    die();
 }
...
<div class="box-content">
	...
	<?php
		if(isset($_POST['acao'])){
			$empreendId = $id;
			$nome = $_POST['nome'];
			$preco = Painel::formatarMoedasBd($_POST['preco']);
			$area = $_POST['area'];
			...
			if($sucesso){
				...
				$sql = Msql::conectar()->prepare("INSERT INTO `tb_admin.imoveis` VALUES (null,?,?,?,?,0)");
				$sql->execute(array($empreendId,$nome,$preco,$area));
				$lastId = Msql::conectar()->lastInsertId();
				foreach ($imagens as $key => $value) {
					Msql::conectar()->exec("INSERT INTO `tb_admin.imagens_imoveis` VALUES (null,$lastId,'$value')");
				}
